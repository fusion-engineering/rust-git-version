...
fn canonicalize_git_path(git_dir: impl AsRef<Path>, file: impl AsRef<Path>) -> syn::Result<String> {
	...
	let path = path.canonicalize().map_err(|e| error!("failed to canonicalize {}: {}", path.display(), e))?;
...
fn git_dependencies() -> syn::Result<TokenStream2> {
	...
	let warning = head.as_ref().err().or(index.as_ref().err()).map(|error|
		warning!("Failed to add dependencies on the git state: {}. The crate may not rebuild if the git state changes.", error)
	);
...
fn git_describe_impl<I, S>(args: I) -> syn::Result<TokenStream2> where
